...
public class Human implements Sport {
    ...
    public void jump(Wall wall) {
        if (wall.getHeight() > jumpLimit) {
            isDead = true;
            System.out.printf("Человек %s не смог подпрыгнуть на %f метров и сдох %n", this.name,wall.getHeight());
            return;
        }
        System.out.printf("Человек %s подпрыгнул на %f метров %n", this.name, wall.getHeight());
    }
    ...
    public void run(Track track) {
        if (track.getLength() > runLimit) {
            isDead = true;
            System.out.printf("Человек %s не смог пробежать %f метров и сдох %n", this.name,track.getLength());
            return;
        }
        System.out.printf("Человек %s пробежал %f метров %n", this.name, track.getLength());
    }
